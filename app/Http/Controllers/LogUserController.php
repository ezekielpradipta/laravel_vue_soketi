...
class LogUserController extends Controller
{
    ...
    {
        $filter_email = $request->username ?? "";
        $filter_type = $request->filter ?? "";
        $filter_status = $request->status ?? "";
        $tanggal_awal = $request->tanggal_awal ?? "";
        $tanggal_akhir = $request->tanggal_akhir ?? "";
        $query = DB::table('log_users')->orderBy('created_at', 'desc')->whereRaw("filter like '%$filter_type%' AND username like '%$filter_email%' AND status like '%$filter_status%'  ");
